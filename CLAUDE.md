...
- `src/content/` holds five Keystatic-managed Content Collections (config: `src/content.config.ts`): `legal` (privacy/terms/transparency long-form prose, rendered by `privasi/syarat/transparansi.astro`), `faqs`, `programs`, `footer-cols`, and `settings`. All are editable via the Keystatic admin UI at `/keystatic` (`src/pages/keystatic/`).
