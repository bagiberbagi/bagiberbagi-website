...
- `src/lib/`: pembaca data & fungsi murni: program (`programs.ts`), rekam jejak (`jejak.ts`), organisasi (`organisasi.ts`), agregasi dampak (`impact.ts`), sorotan beranda (`home.ts`), cara ikut & kartu donasi (`aksi.ts`, `ajakan.ts`), ketentuan program (`ketentuan.ts`), format Rupiah & link WhatsApp (`format.ts`), koordinat lokasi (`geo.ts`), embed video (`video.ts`), foto (`photos.ts`, `assets.ts`), potongan schema.org (`schema.ts`), resolusi SEO per halaman (`seo.ts`), gambar share (`share-image.ts`), switchboard analytics (`analytics.ts`). Yang punya unit test: `aksi`, `format`, `geo`, `impact`, `ketentuan`, `photos`, `video`.
- `src/scripts/`: JS interaktif per fitur (mobile nav, header saat di-scroll, mega-menu program, hitung mundur hero, count-up angka, kartu donasi, band program di beranda, akordeon FAQ, fade-in saat di-scroll, lightbox galeri, peta lokasi jejak, facade embed video, tombol share, TOC legal).
