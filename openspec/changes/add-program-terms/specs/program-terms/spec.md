...
to know before taking part: how the donation is processed, what it covers, what report follows,
what happens when a distribution cannot run, and whatever else is specific to that programme.
...
- **WHEN** a visitor opens an active programme page
...
- **WHEN** a programme declares a term whose title matches a shared term, e.g. "Laporan penyaluran"
...
- **WHEN** a programme declares two terms of its own and the shared block holds seven
- **THEN** the page SHALL show two open terms followed by seven collapsed headings
