...
**Status: shipped.** All eleven tracks merged into `main` and deployed on 6 August 2026
(`2ab8471..69a043e`, deploy run 31137734925). Every box below is ticked, but three of them are
ticked as *decided against* rather than *built* — see the outcome notes on Track F and on D3.
The two statements this file got wrong are corrected inline where they appear, not silently
edited away, because the estimate being wrong is the reason the owner had to be asked at all.
...
- [x] A1.1 `Header.astro:180` — give `#mobile-nav-panel` a height cap and its own scroll:
...
- [x] A1.2 Add a `dvh` fallback for older Safari: declare `100vh` first, then `100dvh`.
- [x] A1.3 Verify at 320×568, 390×844 and 430×932, submenu open, that the last target
...
- [x] A1.4 Verify the panel still fits without a scrollbar when the submenu is closed. It was
      298px tall; nothing should change in that state.
- [x] A1.5 Verify scrolling inside the panel does not scroll the page behind it, and that the
...
- [x] A2.1 `Header.astro:66` — add `widths` and `sizes` to the `<Image>`. Header slot is 201px,
      so `widths={[201, 402, 603]}` with `sizes="201px"` is sufficient. Keep `class="h-9 w-auto"`.
- [x] A2.2 `Footer.astro:68` — same treatment for the 179px slot. **This is the only line in
      `Footer.astro` this track may touch**; Track F owns the rest of that file.
- [x] A2.3 Confirm the emitted `srcset` is non-empty and that the file fetched at DPR 1 differs
...
- [x] A3.1 Remove `relative` from the class list.
- [x] A3.2 Confirm `getComputedStyle(document.querySelector('[data-header]')).position` is still
...
- [x] B1 Move the upload target to `src/assets/organisasi/`, mirroring the programme photo
...
- [x] B2 Add the resolver via `createImageResolver` in `src/lib/assets.ts`, the same shared
      helper `programs.ts` and `jejak.ts` already use. Do not write a fourth private copy.
- [x] B3 `src/content.config.ts` — the logo field becomes an image module rather than a string.
      Keep it nullish, not optional: Keystatic writes `null` when an image field is cleared.
- [x] B4 `src/lib/organisasi.ts` — the reader returns the module.
- [x] B5 `OrganisasiCard.astro:138-145` — swap the plain `<img>` for `<Image>` with
...
- [x] B6 Move the existing file, then confirm `/organisasi/`, `/organisasi/46cyclist/` and
      `/jejak/` all still render the logo and all still return 200.
- [x] B7 Report the served byte size before and after at viewport 320. It must drop by at least
      an order of magnitude.
- [x] B8 **A missing file must warn and fall back, not fail the build**, matching how
      `jejak.ts` handles a stale path. One bad content entry must not take the site down.
- [x] B9 Update the "deliberately not routed through `astro:assets`" sentence in
...
- [x] C1 `Faq.astro:45` — make the wrapper a `<label for="faq-search">`, or nest the input in
...
- [x] C2 Confirm `document.activeElement` becomes the input after clicking 2px inside the
      wrapper's top edge.
- [x] C3 Confirm the accessible name is still "Cari Topik" and has not been duplicated or lost.
...
- [x] D1 `tailwind.config.mjs:81-83` — add `wide: '900px'` next to `nav: '860px'`.
- [x] D2 Replace the raw `900px` in `TrustSection.astro:89,93,98,105`, `ImpactSection.astro:213`,
...
- [x] D3 **Prove the rendered output is unchanged.** Build before and after, then diff
      `dist/**/*.html` and the emitted CSS. ~~Any difference at all is a mistake in this track,
      not an improvement.~~

      **This requirement was wrong and the track proved it.** Registering *any* screen makes
      Tailwind emit one more `.container` rule, so the CSS necessarily grows by exactly 50 bytes:

      ```
      @media (width>=900px){.container{max-width:900px}}
      ```

      `.container` is used nowhere in this codebase, so the rule is inert, and Tailwind already
      emits the identical dead rule for every other screen including `nav`. The achievable and
      meaningful bar is the one that actually shipped: **every HTML file byte-identical once the
      CSS filename hash is normalised**, verified by diffing the two builds. A spec that demands
      byte-identity from a config change should say which bytes, or it forces the agent to either
      fail a passing track or quietly redefine the gate.
- [x] D4 Note in `tailwind.config.mjs` that the breakpoint vocabulary lives here while the
...
- [x] E1 `LegalLayout.astro:51` — `md:w-[240px]` is a raw arbitrary width on a page shell, which
...
- [x] E2 Delete `src/components/WhyFood.astro`. `ProgramStage.astro:3` states it replaced it and
...
- [x] E3 `BaseLayout.astro:231` — keep `overflow-x-hidden` on `<body>`, but write down why it is
...
- [x] E4 Confirm the legal pages render identically at 768 and 1024 before and after E1.
...
- [x] F1 Link stack. Measured: `link self-start text-body-sm ... py-1.5` renders 33.7px tall
      with a 0.3px gap to the next link, and two measured pairs sit at −0.7px, meaning they
      touch. Four occurrences of the class, 20+ links, on every page. ~~`py-2.5` brings the box
      to roughly 44px.~~

      **Two things wrong here, and the first caused the second.** `py-2.5` measures **41.69px**,
      not "roughly 44px" — an estimate written into a spec as if it were a measurement, which is
      how a track can be delivered, reviewed and still be under the bar it was written to clear.
      `py-3` is what actually reaches 45.69px.

      **Then the owner rejected the change outright**, so the padding is back at `py-1.5` and
      footer links deliberately stay at **33.7px**. Clearing 44px added ~120px to the mobile
      footer and he refused that trade. This is a decision, not an oversight: the links are
      stacked text in a low-traffic region, and the cost landed on every page.

      Live consequence to keep in view: `design-system`'s 44px sentence and this code now
      contradict each other. Either that spec gains a carve-out for stacked text links, or this
      comes back. Recorded in the `project-navbar-mobile-backlog` memory, not resolved here.
- [x] F2 **The consequence needs the owner's eyes**: the mobile footer grows by roughly 200px.
      That may be fine, or it may argue for two columns instead of four on a phone. Prepare both
      and let the owner pick. — **Picked: neither.** He kept the current density. Track F was
      reset and rebuilt around F3 alone.
- [x] F3 The SEO paragraph at `Footer.astro:156-157` runs 200 characters per line at 1280 and
      ...
      **Shipped differently, on the owner's instruction**: "seo booster bagus kalo mengikuti
      lebar max-width footer, dan bahkan jika perlu alignment nya rata kiri kanan." So the block
      keeps the footer's own `Container` width and gains `md:text-justify` instead. Narrowing it
      to `prose` was built first and rejected on sight — it was the only element in the footer
      stopping halfway while every neighbour spanned full width, which read as a mistake rather
      than as a reading measure.

      Justification is capped at `md` on purpose. Measured leftover space per word gap: 1.44px
      at 1280, 2.91px at 768, **7.79px at 390**. Phones stay ragged-right because justifying a
      12px line that narrow tears rivers through it.
- [x] F4 Decide with the owner whether that block stays 12px. It is search-facing text, so
      small is defensible; 200 characters per line is not. — **Stays 12px** (`text-micro`). F3's
      width and justification fixed the measure, so the size never had to move.
...
- [x] G1 Raise the icon buttons to 44×44 and the gap to at least 8px.
- [x] G2 The row grows from about 170px to about 236px. Confirm it still fits the 280px content
      column at viewport 320 without wrapping.
- [x] G3 If the owner prefers the row to stay visually light, keep the 32px painted circle and
...
- [x] H1 Raise the `.ps-tick` anchor to 44px tall. The painted bar stays 3px, so nothing on
      screen changes size.
- [x] H2 `.ps-nav` sits at `bottom: 16px` on phones. Check the taller anchor does not overlap
      `.ps-cta` above it, and adjust the offset if it does.
- [x] H3 Correct the comment at `:455`, which currently states something false.
- [x] H4 Consider whether phones should get the arrows back rather than relying on swipe plus a
...
- [x] I1 Carousel arrows at `jejak/[slug].astro:311` and `:314` measure 36×36. Raise to 44×44.
- [x] I2 Breadcrumb links measure 53×19.5 and 84×19.5 at
...
- [x] I3 Body text. Measured font-size histogram at viewport 390:
...
- [x] I4 Convert `PintuS1.astro:1300,1402,1439` and `:1974` to the `wide` name from Track D.
...
- [x] J1 Raise `padding: 7px 16px` so the button clears 44px tall.
- [x] J2 The row is `flex-wrap` with an 8px gap; confirm it does not wrap to three lines at
      viewport 320.
...
- [x] K1 Move the hero split to the `wide` threshold from Track D.
- [x] K2 Check the hero's donation card is not too narrow at 900-959. If it is, the correct
      answer may be to move the whole cluster to 960 rather than the hero to 900. Measure both
      and let the owner choose.
- [x] K3 `Hero.astro:91` uses `860px`, which correctly matches `screens.nav`. Leave it, and add
...
## The owner's calls on Group 2

| track | asked | answered |
|---|---|---|
| F | keep density or clear 44px | **keep density**; see F1–F4 above |
| G | full 44px button, or 32px circle with an invisible hit area | **full 44px** ("ikut usulanmu"). `Share.astro:23` is `w-11 h-11` |
| H4 | give phones the arrows back? | not taken; the ticks are 44px now and the band gained a caption row, so the hairline is no longer the only affordance |
| I3 | promote paragraph text on programme and jejak pages | **taken** (`c38a5fa`) |
| K2 | hero splits at 900, or the whole cluster moves to 960 | **hero moves to 900** — "kiri kanan hanya jika tidak rusak, tpi experience harus tetap relevan" |

## What shipped on top of this change

The owner reviewed on an iPhone while Group 2 was in flight and raised nine more findings, then
four more after that. They were built as separate branches in the same push rather than folded
into these tracks, because none of them were in the audit this change was written from:

- hero link underline, spanning 183.9px of box against 156.9px of text → dropped at rest
- programme stage: caption moved below the indicator, green pulse dot and its keyframes deleted
- FAQ: per-item rules hidden below `lg`, and the two competing CTAs given a hierarchy
- pintu cards to one column below `sm`, status moved into the foot row, forced height removed
- navbar: round transparent hamburger, floating panel shadow, compact 50px pintu rows, Jumat
  Berkah restored as a real card carrying `getProgramCover()` — panel 1481px → 798px
- the donation card lost its default 6 porsi; presets became 6/12/20 with nothing preselected
- structured section ids across every page, plus `.claude/rules/section-ids.md`

## Defects this change introduced and then closed

Worth recording, because all three were caused by our own fixes rather than found by the audit:

1. **Track I's breakpoint move broke `ProgramCover`'s `sizes`.** Its middle branch still said
   `(min-width: 768px) 50vw`, so between 768 and 899 the browser fetched a 384px file for a
   688px box. Fixed to `900px`.
2. **`DonationCard`'s `sizes` went stale the same way**, `(min-width: 960px)` against a hero that
   now splits at 900.
3. **`#donasi` had no scroll offset.** It sits inside `Hero`, which looked like the documented
   "first thing on the page" exemption, but at 390 the card is 464–591px down and a jump from
   the FAQ landed 80–84px under the sticky pill. `.claude/rules/section-ids.md` was corrected the
   same day it was written: **the exemption is about document position, not about which component
   the element lives in.**

## Proof

- 16 routes × 8 widths = 128 cells: **0 overflow, 0 http errors**
- touch targets under 44px, live versus local: **1102 → 1000, no route worse**
- 18-agent adversarial preflight review: **0 findings**
- gate at merge: `astro check` 0 errors, `bun test` 41 pass, 53 pages built, PNG guard 0

## Known and deliberately not fixed

`thumbnailUrl` in the jejak JSON-LD reads `.src` off an image module, which marks the source as
used directly and copies ~372 KB of raw JPEG into `dist`. The repo's cheap guard only counts
`*.png` while 22 of 24 jejak sources are jpg or jpeg, so it stays silent. Out of scope here;
the guard's file-extension blind spot is the more useful half of the finding.
