...
- [x] 6.4 Add `src/pages/keystatic/[...params].astro` admin route — manual `<Keystatic config client:load />` mount (not the `keystatic()` integration, which would force an adapter), with `getStaticPaths()` for the static build and a `Config` type assertion for a known Keystatic generics/TS-variance issue
- [ ] 6.5 Sign up at keystatic.cloud, create a project, connect this GitHub repo, replace the `TODO/bagiberbagi-website` placeholder in `keystatic.config.ts` with the real `cloud.project` slug (manual, outside-repo step — needs user action)
