...
- [x] 6.6 Applied the nginx SPA-fallback `location /keystatic/ { try_files $uri $uri/ /keystatic/index.html; }` block manually into the live VPS config (inserted, not overwritten whole-file, since certbot had already added SSL/redirect blocks), `nginx -t` + `systemctl reload nginx` done
